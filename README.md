...
旧エンジンは再検索のたびにページ全体がリロードされるため、スイープは
**リロードをまたぐイベント駆動の状態機械** (`src/background/legacy-sweep.ts`) で制御します:

1. 期間×曜日×帰国オフセットから検索ジョブ列を生成（`date-matrix.ts`）
2. content script が再検索フォームに日付を投入して検索（`legacy-driver.ts` の `legacySubmit`）→ ページ遷移
3. 新しい結果ページ読込で content script が解析し、日付ペアと行を SW に報告
4. SW のコントローラが「期待した日付ペアか」を照合 → 一致したら結果を記録し、
   スロットル（ジッタ付き）待機後に次のジョブを投入。タイムアウト時はリトライ→失敗扱いで継続

**前提・制約**:
- スイープ開始時に**ANAの国際線特典 往復空席照会の結果ページを開いた状態**である必要があります
  （最初の再検索フォームがそのページに存在するため）。サイドパネルは開いたままにしてください。
- 再検索フォームのセレクタ（JSF自動採番ID・確認ダイアログ・Akamai bot検知）は実機での
  微調整が必要な場合があります。投入に失敗するとタイムアウト→リトライし、続行できなければ
  該当ジョブを失敗として次へ進みます（「失敗分を再実行」で後追い可能）。
- 制御ロジック（ジョブ進行・リトライ・一致照合）は `test/legacy-sweep.test.ts` で検証済み。
